...
handlers[101] = function(params)
    ...
    local charRes = db.query(string.format([[
        ...
        EXTRACT(EPOCH FROM ("LastLogin" - "Datestamp"))/60 AS "survTime",
        EXTRACT(EPOCH FROM (NOW() - "LastAte"))/60 AS "lastAte",
        EXTRACT(EPOCH FROM (NOW() - "LastDrank"))/60 AS "lastDrank",
        "Model"
        FROM "Character_DATA"
...
handlers[202] = function(params)
    ...
    db.execute(string.format([[
        ...
        SET "Alive" = 0, "LastLogin" = CURRENT_TIMESTAMP - (%s * INTERVAL '1 minute')
